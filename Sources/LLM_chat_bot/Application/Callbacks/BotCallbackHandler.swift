...
Оплатить можно звёздами Telegram, картой или криптовалютой — доступные способы бот покажет сам. При оплате криптой отправьте <b>ровно</b> ту сумму, что он показал: именно по ней бот узнаёт ваш платёж. Для покупки нужен <code>@username</code> — задайте его в настройках Telegram, если ещё нет.
