...
enum SuperHelpSection: String, CaseIterable, Sendable {
    ...
    var body: String {
        ...
        case .payments:
            ...
            UI: «🏦 Внешняя касса» в супер-меню. Один агрегатор закрывает то, чего нет в Telegram: Сбербанк Онлайн, СБП, карты РФ и зарубежные, крипта.

            Порядок: ID магазина → секретное слово → секретное слово 2 → валюта → цена подписки и курс пополнений → «↺ Стандартные» способы → «🟢 Включить».

            <b>URL оповещения</b> со страницы кассы нужно вставить в кабинет агрегатора — без него оплата пройдёт, а доступ не включится. Метод оповещения — POST или GET, оба принимаются.

            Способы оплаты — это коды платёжных систем агрегатора (<code>i</code>): их набор правится кнопками, релиз для этого не нужен. Пустой список = покупатель выбирает способ на странице кассы.

            Инструкция по регистрации — PAYMENTS_SETUP.md.
            """
