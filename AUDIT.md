...
>
> Остаётся известным поведением: разрез может пройти между `<b>` и `</b>` —
> форматтер закроет тег в первом куске, а непарный `</b>` во втором выбросит,
> поэтому продолжение теряет **это** оформление (текст цел). Перенос открытых
> тегов через границу — отдельная задача.
