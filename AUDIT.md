...
> ✅ **Исправлено** (вариант 1, но без правки восьми словарей). У ожидания
> появился владелец: `notePendingInputOwner` пишется после каждого действия меню,
> `processTextInput` сверяет `pendingInputOwner` с отправителем и отдаёт чужое
> сообщение дальше (`return false`). Заодно три ветки, молча глотавшие сообщение
> (`return true` без ответа), теперь отвечают.
...
> ✅ **Исправлено** (вариант 1 + страховка варианта 2). `MessageSplitter`
> научился считать **рендер**: `renderedLength` (пессимистичная оценка — `&` = 5,
> `<`/`>` = 4) и `splitRendered` (режет по тому же правилу «перевод строки →
> пробел → жёсткий рез», но бюджет — в экранированных символах). Оценка
> сознательно завышена: настоящий `<b>` считается как экранированный, поэтому
> ошибка всегда в сторону «разрезали раньше», а не «сообщение не влезло».
> `GenerationCoordinator` (оба режима стриминга) budget'ит по `renderedLength`;
> `TelegramHTTPGateway.chunkFittingHTML` дополнительно **проверяет настоящим
> форматтером** и вдвое ужимает бюджет, пока рендер не влезет в 4096 — так учтены
> и теги, которые дописывает сам форматтер. `editMessage` (правку нельзя разбить
> на два сообщения) через него же подрезает патологический вход: показать почти
> всё лучше, чем получить 400 и не показать ничего.
>
> **Перенос открытых тегов через границу тоже сделан** (вторая половина пункта):
> `splitRendered` сдвигает границу перед висящей разметкой (`<…>` / `&…;`),
> `openTagMarkup` переоткрывает в продолжении всё, что осталось открытым, а
> `closingTagMarkup` закрывает это перед «↓ продолжение ниже», стоп-нотисом и
> футером. Стек зеркалит `TelegramHTMLFormatter`; `script`/`style` не
> переоткрываются. Проверено на реальном форматтере: `<b>`, `<a href>`,
> `<blockquote expandable>`, `<pre><code class="language-…">`, вложенность
> `<b><i>`, непарный `</b>` в середине, разрез внутри тега и внутри `&amp;`.
...
> ✅ **Исправлено** (все подпункты).

1. ✅ **`ModelPriceMonitor`** — текст переписан честно: с премиумом/балансом ничего
   не меняется, без них модель работает в пределах дневной порции, а подмена на
   бесплатную случается, когда порция кончится.
2. ✅ **`superAdminPrivateChats()`** теперь резолвит личку **каждого** суперадмина
   через `privateChatID(forKey:)` (заблокировавшие бота отсеиваются), а не чаты,
   привязанные к root-тенанту.
3. ✅ **Access-gate** разбирает текст `CommandParser`'ом (с учётом `@botname` и
   тест-суффикса) вместо `hasPrefix` — `/buying` и `/startsomething` мимо гейта
   больше не проходят.
4. ✅ **Исправлено вместе с B4.** ~~При неудачном `setWebhook` уходим в polling,
   но `deleteWebhook()` не зовём.~~
5. ✅ **Typing** — поведение оставлено как есть (оно верное: дальше прогресс видно
   по draft'у или правкам), поправлены комментарий в коде и CLAUDE.md §9.
6. ✅ **`splitAndContinue`** бросает свой `ContinuationUnavailable` вместо
   `CancellationError`: уже отправленный текст сохраняется, а вместо ложного
   «⏹ Остановлено» приписывается «не удалось отправить продолжение».
7. ✅ **`UserBalance`** — доккомментарий про `UserKey`.
