...
- Провал flush → батч кладётся в `retryCarry` и **сливается**
  (`PersistenceBatch.merged`, newer wins) со следующим; delete в новом батче
  отменяет upsert из старого и наоборот, поэтому ретрай не воскрешает удалённое.
...
Метода «списать баланс» вне транзакции не существует — поэтому платёжный путь
не может «забыть» durability, как мог, пока правило было фразой в документации.
...
В `bot_config` осталось то, что действительно документ (`GlobalConfigKey`, 20
...
проверяется на **каждом** входе в кассу: `/buy`, страница `pay`,
`pre_checkout_query`. Не durable → не продаём: `answerPreCheckoutQuery(ok: false)`
показывает покупателю текст и **не списывает деньги**. Это единственное место в
продукте, где «не продать» строго лучше, чем «продать».
...
- **`STATE_ENCRYPTION_KEY`** — 32 байта base64 (`openssl rand -base64 32`).
  ...
  открытом виде хуже, чем не запуститься. Потеря ключа стоит переввода
  реквизитов из кабинета вендора, не состояния.
...
- **Начисление и отметка «выплачено» — не одно и то же место.** Отметка
  ...
  свой ключ.
- **Платёжный секрет в базе оборачивается `SecretBox`** (§5.6): шифруется на
  входе в `Codable` конфига, наружу отдаётся открытым только тому, кто явно его
  спросил. Новое поле такого рода обязано пройти тем же путём — иначе дамп базы
  это выданные платёжные реквизиты.
...
`swift test` — цель `LLM_chat_botTests` (`Tests/LLM_chat_botTests/`), 342 теста.
327 из них — чистая логика без сети: стор поднимается в памяти
(`Fixtures.makeStore()`), база, Telegram и провайдеры не участвуют. Оставшиеся
15 (`PostgresIntegrationTests`) требуют настоящий Postgres и **сами себя
...
- `DurabilityTests` — разбор `DATABASE_URL` (включая распознавание
  ...
  стоит ноль, переполнение насыщает вместо ловушки.
- `LedgerTests` — правила про деньги, не зависящие от реализации (§10.2): ключ
  ...
  оплату отмечает только покупка пакета.
- `ConfigRoundTripTests` — **каждый** `GlobalConfigKey` экспортируется и
  кодируется. Единственный класс ошибок, который иначе не ловит ничто: ключ,
  который пишется, но не читается, молча откатывается на дефолт при каждом
  рестарте.
- `PostgresIntegrationTests` — то, что существует **потому что** база
  ...
  суперадмином и winback-скидка переживают рестарт, и полный round-trip
  состояния store → батч → база → restore.
