...
`swift test` — цель `LLM_chat_botTests` (`Tests/LLM_chat_botTests/`), 380 тестов.
361 из них — чистая логика без сети: стор поднимается в памяти
(`Fixtures.makeStore()`), база, Telegram и провайдеры не участвуют. Оставшиеся
19 (`PostgresIntegrationTests`) требуют настоящий Postgres и **сами себя
...
- `PostgresIntegrationTests` — то, что существует **потому что** база
  ...
  restore (остальные строки приезжают), и проигравшая попытка взять блокировку
  **не** сообщает о потере (`onLost` — только про отнятую блокировку).
