...
`super*`-страница обязана быть дописана в этот список.
...
(`_freeModelIDs`) ∪ бесплатные с OpenRouter.
...
ключ протух), а закреплённых моделей по умолчанию нет — тогда
`effectiveFreeModelIDs()` возвращает nil. Раньше гейт в `processContent` в этом
...
- **Кнопка «купить» несёт источник**: `menu:nav:pay:<PurchaseSource>`
  ...
  `referral`, `model` — пикер моделей, где платная модель недоступна). Новая
...
`swift test` — цель `LLM_chat_botTests` (`Tests/LLM_chat_botTests/`), 230 тестов,
...
- `EndToEndTests` + `FakeTelegram.swift` — **сквозные**: апдейт входит в
  ...
  рендер меню и переход на страницу покупки, отказ `super*`-страницы обычному
  юзеру, реферальный диплинк с выплатой после первого ответа, `src_`-метка,
  оффер при исчерпании дневного лимита, тап по примеру, разбивка длинного
  ответа.
