...
- **Оплата засчитывается на всех путях**: `handleSuccessfulPayment` (обе ветки —
  ...
  платежом.
- **Хранение**: одна строка `bot_config` (`Config.trafficSources`);
...
`swift test` — цель `LLM_chat_botTests` (`Tests/LLM_chat_botTests/`), 355 тестов.
338 из них — чистая логика без сети: стор поднимается в памяти
