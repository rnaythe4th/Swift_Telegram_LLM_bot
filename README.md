...
Мультитенантный LLM-чат-бот для Telegram на **Swift 6** (server-side).

Один задеплоенный процесс обслуживает много независимых «виртуальных копий» (тенантов) — у каждой свой владелец,
дефолты, заготовки, лицензии и деньги. Состояние пишется в Postgres и сохраняется после рестартов.

|         |                                                            |
| ------- | ---------------------------------------------------------- |
| Язык    | Swift 6.2, strict concurrency                              |
| Тесты   | 587 XCTest-кейсов, прогоняются на настоящем Postgres       |
